...
var WebPrint = function (init, opt) {
    ...
    this.printSerial = function (data, port) {
        if (isAndroid){
            alert("La impresión por puerto serial no está disponible en Android.");
            return;
        }
    ...
    this.printHtml = function (data, printer) {
        if (isAndroid){
            alert("La impresión HTML no está disponible en Android.");
            return;
        }
    ...
    function sendAppletRequest(data) {
        ...
        if (!wpwindow || wpwindow.closed || !wpready) {
            if (wpready){
               ...
               webprint.checkRelay();
               console.log("No se puede establecer conexión con el applet de impresión...intentando la reconexión.");
            }
            setTimeout(function () {
    ...
    function dispatchWebPrint() {
        var answer = confirm("No se puede conectar con la aplicación de impresión.\nDeseas abrir/instalar la aplicación de impresión?");
        if (answer) {
            if (isAndroid){
                deployAndroid();
                return;
            ...
            window.open("http://tienda.uniformescisne.mx/webprint/"+installFile, '_blank');
        }
    }
    ...
    function deployAndroid(){
        ...
        //document.location.href = "intent://#Intent;scheme=webprint;package=au.com.wallaceit.webprint;S.browser_fallback_url=http%3A%2F%2Fwallaceit.com.au%2Fplaystore%2Fwebprint;end";
    }
    ...
    function deployAndroidChrome(){
        ...
        var html = '<div id="intent_link" style="position: fixed; top:40%; width: 120px; background-color: white; left:50%; margin-left: -60px; border: solid 2px rgb(75, 75, 75); font-family: Helvetica SansSerif sans-serif; text-align: center; padding: 5px;">' +
            '<a onclick="window.location=\'intent://#Intent;scheme=webprint;package=au.com.wallaceit.webprint;S.browser_fallback_url=http%3A%2F%2Fwallaceit.com.au%2Fplaystore%2Fwebprint;end\'; document.getElementById(\'intent_link\').remove();">Click para abrir WebPrint</a></div>';
        document.body.innerHTML += html;
    }

    function deployAndroidFirefox() {
        var timeout = setTimeout(function() {
            window.location = "http://wallaceit.com.au/playstore/webprint";
        }, 1000);
