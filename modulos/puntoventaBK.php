...
if ($_POST["accion"]=="cobrar") {
    if (mysqli_num_rows(mysqli_query($con,"select * from trcuentaproductostmp where idvendedor='".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."'"))>0) {
        ...
        // tcuentas 
        // si ya se tiene un idcuenta, sólo se actualiza, no se inserta
        if ($_POST["idcuenta"]>0) {
            mysqli_query($con,"update tcuentas set idcorte = '".$idcorte."', descuento = '".$_POST["txtDescuento"]."', subtotal = '".$_POST["subtotal"]."', iva = '".$_POST["iva"]."', total = '".$_POST["total"]."' where idcuenta = '".$_POST["idcuenta"]."'");
            $idcuenta = $_POST["idcuenta"];
        } else {
            mysqli_query($con,"insert into tcuentas (idsucursal,idcorte,idvendedor,tipocuenta,nombrecuenta,descuento,subtotal,iva,total,fecha,status) values ('".$vendedor["idsucursal"]."','".$idcorte."','".$vendedor["idvendedor"]."','D','".$_POST["nombrecuenta"]."','".$_POST["txtDescuento"]."','".$_POST["subtotal"]."','".$_POST["iva"]."','".$_POST["total"]."','".date("Y-m-d H:i:s")."','P')");
        ...
        // ttickets
        mysqli_query($con,"insert into ttickets (idcuenta,idsucursal,idcorte,idvendedor,folio,descuento,subtotal,iva,total,fecha,status) values ('".$idcuenta."','".$vendedor["idsucursal"]."','".$idcorte."','".$vendedor["idvendedor"]."','".($folio)."','".$_POST["txtDescuento"]."','".$_POST["subtotal"]."','".$_POST["iva"]."','".$_POST["total"]."','".date("Y-m-d H:i:s")."','A')");
        $idticket = mysqli_insert_id($con);
        ?>
        <script>
        $(document).ready(function(e) {
            imprimirTicket(<?php echo $idticket; ?>,<?php echo $_POST["cambio"]; ?>);
        });
        </script>
        <?

        // si se elimina algun producto, se debe quitar de la tabla permanente y devolver las existencias
        $cuentaproductos = mysqli_query($con,"select * from trcuentaproductostmp where idvendedor = '".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."' and borrar=1");
        while ($cuentaproducto = mysqli_fetch_assoc($cuentaproductos)) {            
            // devolver las existencias del producto con idcolor, idtalla y cantidad seleccionados
            ...
            // quitar el producto
            mysqli_query($con,"delete from trcuentaproductos where idcuentaproducto='".$cuentaproducto["idcuentaproducto"]."'");
        }        
        ...
        $cuentaproductos = mysqli_query($con,"select * from trcuentaproductostmp where idvendedor = '".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."' and idcuenta=0");
        while ($cuentaproducto = mysqli_fetch_assoc($cuentaproductos)) {
...
if ($_POST["accion"]=="apartar") {
    if (mysqli_num_rows(mysqli_query($con,"select * from trcuentaproductostmp where idvendedor='".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."'"))>0) {
        ...
        $idcuenta = mysqli_insert_id($con);
        // ttickets
        mysqli_query($con,"insert into ttickets (idcuenta,idsucursal,idcorte,idvendedor,folio,descuento,subtotal,iva,total,fecha,status) values ('".$idcuenta."','".$vendedor["idsucursal"]."','".$idcorte."','".$vendedor["idvendedor"]."','".($folio+1)."','".$_POST["txtDescuento"]."','".$_POST["subtotal"]."','".$_POST["iva"]."','".$_POST["abono"]."','".date("Y-m-d H:i:s")."','A')");
        $idticket = mysqli_insert_id($con);
...
if ($_POST["accion"]=="corte") {
    ...
    $corte = mysqli_fetch_assoc(mysqli_query($con,"select * from tcortessucursales where idvendedor='".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."' order by idcorte desc limit 1"));
    $fondoinicial = $corte["fondoinicial"];
    
    $ticketfinal = mysqli_fetch_assoc(mysqli_query($con,"select * from ttickets where idcorte='".$corte["idcorte"]."' order by idticket desc limit 1"))["idticket"];
    $ticketinicial = mysqli_fetch_assoc(mysqli_query($con,"select * from ttickets where idcorte='".$corte["idcorte"]."' order by idticket limit 1"))["idticket"];
    $ventas = mysqli_fetch_assoc(mysqli_query($con,"select sum(total) as ventas from ttickets where idcorte='".$corte["idcorte"]."'"))["ventas"];
    $devoluciones = mysqli_fetch_assoc(mysqli_query($con,"select sum(total) as devoluciones from trticketproductos where idticketproducto in (select idticketproducto from tdevoluciones) and idticket in (select idticket from ttickets where idcorte='".$corte["idcorte"]."')"))["devoluciones"];
    $fondofinal = $fondoinicial - $devoluciones + $ventas;

    // gastos 
    $efectivo = mysqli_fetch_assoc(mysqli_query($con,"select sum(monto) as monto from tformaspagoticket where idticket>='".$ticketinicial."' and idticket<='".$ticketfinal."' and idformapago=1"))["monto"];
    $efectivousd = mysqli_fetch_assoc(mysqli_query($con,"select sum(monto) as monto from tformaspagoticket where idticket>='".$ticketinicial."' and idticket<='".$ticketfinal."' and idformapago=2"))["monto"];
    $tarjeta = mysqli_fetch_assoc(mysqli_query($con,"select sum(monto) as monto from tformaspagoticket where idticket>='".$ticketinicial."' and idticket<='".$ticketfinal."' and idformapago=3"))["monto"];
    // $gastos = 
    mysqli_query($con,"update tcortessucursales set ticketinicial='".$ticketinicial."', ticketfinal='".$ticketfinal."', ventas='".$ventas."', devoluciones='".$devoluciones."', gastos='".$devoluciones."', efectivo='".$efectivo."', efectivousd='".$efectivousd."', tarjeta='".$tarjeta."',  fondofinal='".$fondofinal."', fechafinal='".date("Y-m-d H:i:s")."', status='T'  where idcorte='".$corte["idcorte"]."'");
}

// codigo
if ($_POST["accion"]=="guardarcuenta") {    
    if (mysqli_num_rows(mysqli_query($con,"select * from trcuentaproductostmp where idvendedor='".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."'"))>0) {
        $vendedor = mysqli_fetch_assoc(mysqli_query($con,"select * from tvendedores where idvendedor = '".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."'"));
        ...
        // si ya se tiene un idcuenta, sólo se actualiza, no se inserta
        if ($_POST["idcuenta"]>0) {
            mysqli_query($con,"update tcuentas set descuento = '".$_POST["txtDescuento"]."', subtotal = '".$_POST["subtotal"]."', iva = '".$_POST["iva"]."', total = '".$_POST["total"]."' where idcuenta = '".$_POST["idcuenta"]."'");
            $idcuenta = $_POST["idcuenta"];
        } else {
            mysqli_query($con,"insert into tcuentas (idsucursal,idcorte,idvendedor,tipocuenta,nombrecuenta,descuento,subtotal,iva,total,fecha,status) values ('".$vendedor["idsucursal"]."','".$idcorte."','".$vendedor["idvendedor"]."','','".$_POST["nombrecuenta"]."','".$_POST["txtDescuento"]."','".$_POST["subtotal"]."','".$_POST["iva"]."','".$_POST["total"]."','".date("Y-m-d H:i:s")."','P')");
            $idcuenta = mysqli_insert_id($con);
        }
        ...
        $idalmacen = mysqli_fetch_assoc(mysqli_query($con,"select * from tsucursales where idsucursal='".$vendedor["idsucursal"]."'"))["idalmacen"];
        // trcuentaproductos
        // al guardar la cuenta por segunda vez o posterior, sólo se añaden los productos que son nuevos
        $cuentaproductos = mysqli_query($con,"select * from trcuentaproductostmp where idvendedor = '".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."' and idcuenta=0");
        while ($cuentaproducto = mysqli_fetch_assoc($cuentaproductos)) {            
            // añadir el producto
            mysqli_query($con,"insert into trcuentaproductos (idcuenta,idproducto,idcolor,idtalla,cantidad,descuento,subtotal,iva,total,status) values ('".$idcuenta."','".$cuentaproducto["idproducto"]."','".$cuentaproducto["idcolor"]."','".$cuentaproducto["idtalla"]."','".$cuentaproducto["cantidad"]."','".$cuentaproducto["descuento"]."','".$cuentaproducto["subtotal"]."','".$cuentaproducto["iva"]."','".$cuentaproducto["total"]."','E')");

            // cuando se guarda la cuenta, las existencias se reducen al momento
            // descontar las existencias del producto con idcolor y idtalla seleccionados
            mysqli_query($con,"update tproductoexistencias set existencia=existencia-".$cuentaproducto["cantidad"]." where idproducto='".$cuentaproducto["idproducto"]."' and idcolor='".$cuentaproducto["idcolor"]."' and idtalla='".$cuentaproducto["idtalla"]."' and idalmacen='".$idalmacen."'");
        }
    }
}
...
// codigo
// si se accede al punto de venta con un idcuenta (cuenta guardada), se deben pasar los registros de productos (trcuentaproductos) a la tabla temporal (trcuentaproductostmp)
if ($_GET["idcuenta"]>0) {
    // trcuentaproductos
    $cuentaproductos = mysqli_query($con,"select * from trcuentaproductos where idcuenta = '".$_GET["idcuenta"]."'");
    while ($cuentaproducto = mysqli_fetch_assoc($cuentaproductos)) {            
        // añadir el producto
        mysqli_query($con,"insert into trcuentaproductostmp (idcuenta,idcuentaproducto,idvendedor,idproducto,idcolor,idtalla,cantidad,descuento) values ('".$_GET["idcuenta"]."','".$cuentaproducto["idcuentaproducto"]."','".$_SESSION["v3nd3d0rpl4y3r4spvc1sn3usr"]."','".$cuentaproducto["idproducto"]."','".$cuentaproducto["idcolor"]."','".$cuentaproducto["idtalla"]."','".$cuentaproducto["cantidad"]."','".$cuentaproducto["descuento"]."')");
    }    
}
?>
...
function sumarProducto(idpartida) {
    if (validarExistencias(0,0,0,idpartida)){
        $.ajax({
            type:"POST",
            url:"/modulos/puntoventa/listaProductos.php",
            data: "accion=sumar&idpartida=" + idpartida,
            success: function(data){
                $("#listaProductos").html(data);
            }
        });
...
function limpiar() {
    <?
    // si es una cuenta guardada, no se puede eliminar
            ...
            if (data=="OK") {
                Swal.fire({
                    ...
                    <div class="row">\
                        ...
                        <div class="col-6">\
                            <input type="text" name="txtMonto" id="txtMonto" class="form-control" placeholder="Monto" onkeypress="verificarPago(event);">\
                        </div>\
                    </div>\
                    <div id="listaPagos">\
                    </div>\
    ...
    $.ajax({
        ...
        success: function (data){
            ...
            Swal.fire({
                title: 'Corte',
                html: '\
                <b>A continuacion se muestra el desglose del corte</b><br>\
                <div class="row"> \
                    <div class="col">\
        ...
        $.ajax({
            ...
            success: function (data){
                if (data=="OK") {
                    ...
                    alert("ATENCION: no se puede apartar si hay al menos un producto personalizado");
                }
            }
        });
        <?
                        ...
                        success: function(data){
                            ...
                            if (datos[0]=="UNCOLORYTALLA") {// tiene solo un color y una talla
                                ...
                                for (i = 0; i < colores.length-1; i++) {
                                    opcionesc += '<option value="' + colores[i].split(".")[0] + '">' + colores[i].split(".")[1] + '</option>';
                                }
                                for (i = 0; i < tallas.length-1; i++) {
                                    opcionest += '<option value="' + tallas[i].split(".")[0] + '">' + tallas[i].split(".")[1] + '</option>';
                                }
                                // swal
                                Swal.fire({
...
function validarFondoInicial() {
    if (validarCantidad($("#txtFondoInicial").val())) {
        $.ajax({
            type:"POST",
...
// codigo
function guardarCuenta(idcuenta){
    // si hay al menos un producto con personalizacion, no se puede guardar la cuenta
    $.ajax({
        type: "POST",
        url: "/modulos/puntoventa/sinpersonalizacion.php",
        success: function (data){
            if (data=="OK") {
                $.ajax({
                    ...
                    success: function(data){
                        if (data=="OK") {
                            // si es una cuenta ya guardada, se guarda con el mismo nombre que se había escrito
                            if (idcuenta>0) {
                                $("#accion").val("guardarcuenta");
                                $("#formPuntoVenta").submit();
                            } else {
                                Swal.fire({
                                    title: 'Escribe el nombre de la cuenta',
                                    html: '<input type="text" id="txtNombre" name="txtNombre" class="form-control">',
                                    inputAttributes: {
                                        autocapitalize: 'off'
                                    },
                                    ...
                                    if (result.value) {
                                        $("#nombrecuenta").val($("#txtNombre").val());
                                        $("#accion").val("guardarcuenta");
                                        $("#formPuntoVenta").submit();
                                    }
                                });
                ...
                alert("ATENCION: no se puede guardar una cuenta con productos personalizados");
            }
        }
    });
}
                ...
                <input type="hidden" name="accion" id="accion" value="fondinicial">
                        <div class="form-group">
                            ...
                            <div class="row mt-3">
								...
								<div class="col-10">
									<input type="text" name="txtFondoInicial" id="txtFondoInicial" class="form-control">
								</div>
							</div>
						</div>
            ...
            <form name="formPuntoVenta" id="formPuntoVenta" method="post" action="home.php?modulo1=puntoventa">
                <div class="row">
                    <div class="col-12">
                        ...
                        <!-- si se llega a través de una cuenta guardada, se recibirá un idcuenta -->
                        // codigo
                        <input type="hidden" name="idcuenta" id="idcuenta" value="<? echo $_GET["idcuenta"]; ?>">
                        <input type="hidden" name="nombrecuenta" id="nombrecuenta" value="">
                ...
                <div class="row" style="margin-top:30px;">
                    ...
                    <div class="col text-right">
                        ...
                        <button type="button" class="btn btn-primary waves-light waves-effect" onClick="apartado();" >Apartar</button>&nbsp;&nbsp;
                        <button type="button" class="btn btn-primary waves-light waves-effect" onClick="guardarCuenta(<? echo $_GET["idcuenta"]; ?>);" >Guardar</button>&nbsp;&nbsp;
...
function validarExistencias(idtalla,idcolor,idproducto,idpartida){
    var correcto = false;
    $.ajax({
        type: "POST",
        url: "/modulos/puntoventa/validarexistencias.php",
        data: "idcolor=" + idcolor + "&idtalla=" + idtalla + "&idproducto=" + idproducto + "&idpartida=" + idpartida,
        async: false,
        success: function (data){
            if (data=="OK") {
                correcto = true;
